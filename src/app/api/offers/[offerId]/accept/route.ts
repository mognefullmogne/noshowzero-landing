/**
 * Public GET — patient clicks Accept link from notification.
...
import { processAccept } from "@/lib/backfill/process-response";
...
export async function GET(
  ...
  try {
    ...
    if (verified.offerId !== offerId || verified.action !== "accept") {
      return htmlResponse(401, "Token non corrispondente", "Il token non corrisponde a questa offerta.");
    }

    const supabase = await createServiceClient();
    const result = await processAccept(supabase, offerId);
